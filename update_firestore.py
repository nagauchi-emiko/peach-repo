      ...
      "elements": [
        {
          ...
          "elements": [
            ...
            {"type": "user", "user_id": "U05R8TB0XR6"},
            {"type": "text", "text": " "},
            {"type": "user", "user_id": "U01VD8QJC0G"},
...
# Firestoreへの初期値の保存実行
def save_to_firestore_initial_settings():
    ...
    db.collection("settings").document("app_config").set({
        ...
        "google_drive_folder_id": "1rZRibqd7q4OXGM7NrdfyfUgriA_gdfKo", # 経理共有ドライブのルートフォルダID(これはダミーデータです)
...
# Firestoreへ投稿メッセージの保存実行
def save_to_firestore_post_message():
...
if __name__ == "__main__":
    # save_to_firestore_initial_settings()
    save_to_firestore_post_message()